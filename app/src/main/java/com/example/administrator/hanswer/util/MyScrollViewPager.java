...
/**
 * 实现页面自动滚动
 * Created by K on 2018/7/4 0004.
 */

public class MyScrollViewPager <T extends PagerAdapter> extends FrameLayout {
    ...
    private static long time = 12000; //自动播放时间
    ...
    public MyScrollViewPager(@NonNull Context context) {
        this(context,null);
    }

    public MyScrollViewPager(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }

    public MyScrollViewPager(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
...
//        viewPager.setPageTransformer(true, new ZoomOutPageTransformer());
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                ...
                Log.e("k","position=="+position+",currentIndex="+currentIndex);
            }

            //即停止滑动的时候滑到指定位置，当手指拖动的时候需要继续自动播放
            @Override
...
//    }
    ...
    public void setAutoPlay(boolean autoPlay) {
        MyScrollViewPager.autoPlay = autoPlay;
        if (!autoPlay){
            handler.removeCallbacks(runnable);
        }
    }
